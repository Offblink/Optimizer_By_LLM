...
| 场景 | 为什么 |
|------|--------|
| 热循环 / 热函数（5% 代码，95% 时间） | 改动小，收益大，验证范围可控 |
| 数值计算、数据处理、加密 | 编译器自动向量化常放弃的领域 |
| Python/JS 等高级语言需要 C 级性能 | 不需要写出等价 C |
| 已知 C 编译器无法向量化的模式 | widen-multiply、gather→load 等 |

**不适用（当前）：**

| 场景 | 为什么 |
|------|--------|
| 全程序优化（>100 函数交互） | 副作用追踪随规模指数级爆炸 |
| 需要形式化正确性保证的系统 | LLM 的验证是统计性的，不是证明性的 |
| IO 密集型程序 | 瓶颈不在 CPU，优化汇编无收益 |
| 无测试覆盖的代码 | 没有验证就没有安全网 |

**核心策略：**

```
profiler → 找到 5% 热路径 → 摘出来 → LLM 优化 → 验证 → 塞回去
```

和人类性能工程师的工作方式一致——没人用汇编重写整个程序，只动那三个热函数。
